...
function createDict(str, del) {
    if (!str) { return; }
    return Object.freeze(str.split(del || '').reduce(function (s, c) {
        s[c] = true;
        return s;
    }, {}));
}
...
function Parser(options) {
    ...
    this._tags = createDict(Array.isArray(options.tags) ? options.tags.join(',') : options.tags, /\s*,\s*/g);
    this._textNode = '';
    this._tagName = '';
    this._attributeName = '';
...
Parser.CharSet = {
    ...
    ALPHANUM:   createDict('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890.')
};
